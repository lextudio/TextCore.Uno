...
namespace LeXtudio.UI.Text.Core
{
    ...
    internal sealed class MacOSTextInputAdapter : IPlatformTextInputAdapter
    {
        ...
        /// <summary>
        /// Called when the editor signals layout changes (WinUI parity). Uno's
        /// native IME requests the latest caret rect via callback, so there is
        ...
        private void SyncSelectionFromContext()
        {
            if (_context is null)
            {
                Log("SyncSelectionFromContext: _context is null");
                return;
            }
            ...
            try
            {
                ...
                Log($"SyncSelectionFromContext: got selection=[{start},{end}) from editor");
                _selectionStart = start;
                _selectionEnd = end;
            }
            catch (Exception ex)
            {
                Log($"SyncSelectionFromContext threw: {ex.Message}");
            }
        }
